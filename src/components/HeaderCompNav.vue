<template>
    <Transition>
        <nav class="absolute h-screen w-screen top-0 left-0 z-10 bg-lime-700 py-6
         md:relative md:h-auto md:w-auto md:z-0 md:bg-transparent md:p-0 md:mr-auto md:self-stretch">
            <HeaderCompNavBackIcon v-if="menu.nav!=''" @click="showSubLinks('')"
            ...
            border-t-2 border-lime-100 md:flex-row md:border-none md:text-left md:gap-5 md:text-lime-800 
             md:font-extrabold md:items-end md:h-full ">
                 <RouterLink v-for="(link,index) in menu.links" @click="$emit('toggleMenuDisplay')" @mouseover="showsublist(link)" :key="index" :to="`/${menu.nav}${link}`" 
                  ...
                  md:p-3 md:hover:border-lime-600  md:border-transparent  md:hover:text-lime-800">
                     {{link}}
                     <HeaderCompNavPlusIcon v-show="link=='categories' || link=='products'" @click.stop="showSubLinks(link)"
                     class=" h-16 absolute right-0 top-0 bottom-0 bg-lime-400 md:hidden"/>
                    </RouterLink>
                </ul>
            </Transition>
            <HeaderCompNavHover v-if="hovered" @hoveredToFalse="hovered=false" @mouseleave="hovered = false" :links="links" :nav="nav"
             class="hidden fixed md:flex flex-col items-center justify-start flex-wrap h-44 top-16 left-0 w-screen z-30 bg-lime-300 bg-opacity-40 border-b-2 border-lime-700" />
        </nav>
    </Transition>
</template>
